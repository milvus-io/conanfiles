...
class JemallocConan(ConanFile):
    ...
    def validate(self):
        ...
        if self.settings.build_type not in ("Release", "Debug", None):
            raise ConanInvalidConfiguration(
                "Only Release and Debug build_types are supported"
            )
    ...
    def _autotools_args(self):
        conf_args = [
            ...
            "--enable-readlinkat"
            if self.options.enable_readlinkat
            else "--disable-readlinkat",
            "--enable-syscall" if self.options.enable_syscall else "--disable-syscall",
            "--enable-lazy-lock"
            if self.options.enable_lazy_lock
            else "--disable-lazy-lock",
    ...
    def _configure_autotools(self):
        ...
        self._autotools = AutoToolsBuildEnvironment(
            self, win_bash=tools_legacy.os_info.is_windows
        )
        self._autotools.configure(
            args=self._autotools_args, configure_dir=self.source_folder
        )
        return self._autotools
    ...
    def build(self):
        ...
        if self.settings.compiler == "Visual Studio":
            ...
            msbuild.build(
                sln_file, targets=["jemalloc"], build_type=self._msvc_build_type
            )
        else:
    ...
    def package(self):
        ...
        if self.settings.compiler == "Visual Studio":
            ...
            self.copy(
                "*.lib",
                src=os.path.join(
                    self.source_folder, "msvc", arch_subdir, self._msvc_build_type
                ),
            ...
            autotools.make(
                target="install_lib_shared"
                if self.options.shared
                else "install_lib_static"
            )
            ...
            if self.settings.os == "Windows" and self.settings.compiler == "gcc":
                rename(
                    self,
                    os.path.join(
                        self.package_folder, "lib", "{}.lib".format(self._library_name)
                    ),
                    os.path.join(
                        self.package_folder, "lib", "lib{}.a".format(self._library_name)
                    ),
                )
                if not self.options.shared:

...
class AwsCIO(ConanFile):
    ...
    def requirements(self):
        ...
        if Version(self.version) <= "0.10.9":
            ...
            self.requires("aws-c-cal/0.5.11@milvus/dev")
        else:
            self.requires("aws-c-common/0.8.2@milvus/dev")
            self.requires("aws-c-cal/0.5.20@milvus/dev")

        if self.settings.os in ["Linux", "FreeBSD", "Android"]:
            self.requires("s2n/1.3.31@milvus/dev")
    ...
    def package_info(self):
        ...
        if self.settings.os == "Windows":
            self.cpp_info.components["aws-c-io-lib"].system_libs = [
                "crypt32",
                "secur32",
                "shlwapi",
            ]
        ...
        self.cpp_info.components["aws-c-io-lib"].names[
            "cmake_find_package"
        ] = "aws-c-io"
        self.cpp_info.components["aws-c-io-lib"].names[
            "cmake_find_package_multi"
        ] = "aws-c-io"
        self.cpp_info.components["aws-c-io-lib"].set_property(
            "cmake_target_name", "AWS::aws-c-io"
        )
        self.cpp_info.components["aws-c-io-lib"].requires = [

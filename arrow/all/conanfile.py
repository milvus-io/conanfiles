...
class ArrowConan(ConanFile):
    ...
    def package_info(self):
        ...
        if self.options.with_azure:
            self.cpp_info.components["libarrow"].requires.extend([f"azure-sdk-for-cpp::{x}" for x in ["azure-storage-common","azure-storage-blobs","azure-identity","azure-storage-files-shares","azure-storage-files-datalake","azure-data-tables"]])
        if self.options.get_safe("with_gcs"):

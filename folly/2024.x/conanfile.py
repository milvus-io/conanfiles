...
class FollyConan(ConanFile):
    ...
    def requirements(self):
        ...
        # liburing removed — not needed by Milvus
    ...
    def validate(self):
        ...
        if boost.options.header_only:
            raise ConanInvalidConfiguration(
                f"{self.ref} could not be built with a header only Boost. Use -o 'boost/*:header_only=False'")

        miss_boost_required_comp = any(
            getattr(boost.options, f"without_{boost_comp}", True)
    ...
    def generate(self):
        ...
        if is_apple_os(self) and cross_building(self):
            for var in ["FOLLY_HAVE_UNALIGNED_ACCESS", "FOLLY_HAVE_LINUX_VDSO",
        ...
        if is_msvc(self):
            cxx_std_value = "c++latest" if str(self.settings.compiler.cppstd) > "17" \
                else f"c++{str(self.settings.compiler.cppstd)}"
    ...
    def _patch_sources(self):
        ...
        replace_in_file(self, folly_deps, "${Boost_LIBRARIES}",
                        f"{' '.join(self._required_boost_cmake_targets)}")
        ...
        # Skip generating .pc file to avoid errors
        replace_in_file(self, os.path.join(self.source_folder, "CMakeLists.txt"),
                        "gen_pkgconfig_vars(FOLLY_PKGCONFIG folly_deps)", "")
        # Remove liburing dependency (not needed by Milvus)
        replace_in_file(self, folly_deps,
                        "find_package(LibUring)", "# find_package(LibUring) -- removed for Milvus",
                        strict=False)

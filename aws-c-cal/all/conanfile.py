...
class AwsCCal(ConanFile):
    ...
    def requirements(self):
        if Version(self.version) <= "0.5.11":
            self.requires("aws-c-common/0.6.11@milvus/dev")
        else:
    ...
    def package_info(self):
        ...
        self.cpp_info.components["aws-c-cal-lib"].names[
            "cmake_find_package"
        ] = "aws-c-cal"
        self.cpp_info.components["aws-c-cal-lib"].names[
            "cmake_find_package_multi"
        ] = "aws-c-cal"
        self.cpp_info.components["aws-c-cal-lib"].set_property(
            "cmake_target_name", "AWS::aws-c-cal"
        )

        self.cpp_info.components["aws-c-cal-lib"].libs = ["aws-c-cal"]
        self.cpp_info.components["aws-c-cal-lib"].requires = [
            "aws-c-common::aws-c-common-lib"
        ]
        if self.settings.os == "Windows":
            self.cpp_info.components["aws-c-cal-lib"].system_libs.append("ncrypt")
        elif is_apple_os(self):
            self.cpp_info.components["aws-c-cal-lib"].frameworks.extend(
        ...
        if self._needs_openssl:
            ...
            if not self.dependencies["openssl"].options.shared:
                ...
                crypto_link_flags = "-Wl," + ",".join(
                    f"-u{symbol}" for symbol in crypto_symbols
                )
                self.cpp_info.components["aws-c-cal-lib"].exelinkflags.append(
                    crypto_link_flags
                )
                self.cpp_info.components["aws-c-cal-lib"].sharedlinkflags.append(
                    crypto_link_flags
                )

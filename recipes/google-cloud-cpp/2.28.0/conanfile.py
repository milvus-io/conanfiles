...
class GoogleCloudCppConan(ConanFile):
    ...
    license = "Apache-2.0"
    topics = ("google", "cloud", "google-cloud-storage", "google-cloud-platform")
    ...
    _SKIPPED_COMPONENTS = {'asset', 'channel', 'storagetransfer'}
    ...
    def validate(self):
        ...
        if self._needs_grpc and self.info.options.shared and \
    ...
    @property
    def _needs_grpc(self):
        selected = self._selected_components()
        return any(c not in ("storage", "oauth2") for c in selected)

    def requirements(self):
        # Dependency versions aligned with Milvus conanfile.py
        ...
        if self._needs_grpc:
            ...
            self.requires("grpc/1.67.1@milvus/dev", transitive_headers=True)

    def build_requirements(self):
        self.tool_requires("cmake/[>=3.17 <4]")
        if self._needs_grpc:
            self.tool_requires("grpc/1.67.1@milvus/dev")
    ...
    def package_info(self):
        ...
        self.cpp_info.components["common"].requires = [
            "abseil::absl_any", "abseil::absl_flat_hash_map", "abseil::absl_memory",
            "abseil::absl_optional", "abseil::absl_time",
        ]
        ...
        self.cpp_info.components["rest_internal"].requires = [
            "common", "libcurl::libcurl", "openssl::ssl", "openssl::crypto", "zlib::zlib",
        ]
        ...
        if needs_grpc:
            ...
            self.cpp_info.components["grpc_utils"].requires = list(GRPC_UTILS_REQUIRED_PROTOS) + [
                "common", "abseil::absl_function_ref", "abseil::absl_memory",
                "abseil::absl_time", "grpc::grpc++", "grpc::_grpc",
            ]
        ...
        if "spanner" in selected:
            self._add_grpc_component("spanner", "spanner_protos", [
                "abseil::absl_fixed_array", "abseil::absl_numeric",
                "abseil::absl_strings", "abseil::absl_time",
            ])
        ...
        if "oauth2" in selected:
            ...
            self.cpp_info.components["oauth2"].requires = [
                "rest_internal", "common", "nlohmann_json::nlohmann_json",
                "libcurl::libcurl", "openssl::ssl", "openssl::crypto", "zlib::zlib",
            ]
        ...
        if "storage" in selected:
            self.cpp_info.components["storage"].requires = [
                "rest_internal", "common", "nlohmann_json::nlohmann_json",
                "abseil::absl_memory", "abseil::absl_strings", "abseil::absl_str_format",
                "abseil::absl_time", "abseil::absl_variant",
                "crc32c::crc32c", "libcurl::libcurl", "openssl::ssl", "openssl::crypto", "zlib::zlib",
            ]

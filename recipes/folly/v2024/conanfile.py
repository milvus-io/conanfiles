...
class FollyConan(ConanFile):
    ...
    def _patch_sources(self):
        ...
        # Fix static_assert failure on glibc where recvmmsg/sendmmsg are weak symbols
        # that GCC cannot evaluate as constant expressions
        netops = os.path.join(self.source_folder, "folly", "net", "NetOps.cpp")
        replace_in_file(self, netops, "static_assert(folly::to_bool(::recvmmsg));", "// static_assert(folly::to_bool(::recvmmsg));")
        replace_in_file(self, netops, "static_assert(folly::to_bool(::sendmmsg));", "// static_assert(folly::to_bool(::sendmmsg));")
